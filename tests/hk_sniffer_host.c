 ...
 *
 * Bug: hk_param_sniffer() accepts any packet whose CSP *source* port is 13 and treats
 * it as an HK param stream. Short packets on sport 13 (e.g. RDP control/ack frames, or
 * any other service that shares the port) carry no param payload. The length math
...
static csp_packet_t *make_pkt(uint16_t sport, uint8_t flags, uint16_t length) {
    ...
    p->id.src   = 42;          /* arbitrary non-zero source node */
    p->id.flags = flags;
    p->length   = length;
    return p;
}
...
int main(void) {
    ...
    /* length 8, RDP: 8 - 5 - 5 underflows. Typical short RDP control/ack frame shape. */
    bug |= run_child("short RDP packet, sport 13 (len=8):", make_pkt(13, CSP_FRDP, 8));
